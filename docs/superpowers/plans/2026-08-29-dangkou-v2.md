...
- [ ] **Step 4: 跑测试通过** → `- [ ]` Run: `python3 -m pytest tests/test_templates.py -v` → PASS

- [ ] **Step 5: Commit** `git add -A && git commit -m "feat: 品类模板常量（唯一事实源）"`
...
- [ ] **Step 5: Commit** `git commit -m "feat: 内部货号 KS-XXXXXXXX"`
...
- [ ] **Step 5: Commit** `git commit -m "feat: 图片存储接口+本地实现"`
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_api.py -v`

- [ ] **Step 5: Commit** `git commit -m "feat: API骨架+图片鉴权服务"`
...
- [ ] **Step 1: 失败测试**（用 fake claude 脚本，不调真模型）
...
- [ ] **Step 3: 实现**（提示词沿用实战验证的硬约束方法论；搬运旧仓库 agent_claude.py v5 结构）
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_agent.py -v`

- [ ] **Step 5: Commit** `git commit -m "feat: Agent解析（模板提示词+超时拯救+防御去重）"`
...
- [ ] **Step 5: Commit** `git commit -m "feat: 导入四分类（新增/更新/下架）"`
...
- [ ] **Step 5: Commit** `git commit -m "feat: 审批工单+一次性token+落库执行"`
...
- [ ] **Step 5: Commit** `git commit -m "feat: 异步导入编排+回调+工单串联"`
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_crud_api.py -v`

- [ ] **Step 5: Commit** `git commit -m "feat: 商品CRUD走审批+导入/工单路由"`
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_h5.py -v`

- [ ] **Step 5: Commit** `git commit -m "feat: H5 审核页+商品页（品类页签+模板CRUD）"`
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_search.py -v`

- [ ] **Step 5: Commit** `git commit -m "feat: 图向量检索（余弦+换一批+入池）"`
...
- [ ] **Step 3: 实现**（api.py 追加 + decide_ticket 增强与 /search 路由）
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_search_api.py tests/ -v`

- [ ] **Step 5: Commit** `git commit -m "feat: 审批后图片落位+向量入池+/search"`
...
- [ ] **Step 4: 通过** → `python3 -m pytest tests/test_quote.py -v`

- [ ] **Step 5: Commit** `git commit -m "feat: 科森模板报价单生成+/quote"`
...
- [ ] **Step 3: 实现**（沿旧仓库 catalog.mjs 的注册契约：apply(ctx) + ctx.registerTool）
...
- [ ] **Step 4: 通过** → `node tests/test_plugin_syntax.mjs` 输出 `plugin ok`

- [ ] **Step 5: Commit** `git commit -m "feat: engine插件catalog-v2（微信入口五工具）"`
...
- [ ] **Step 2: 实现 harness/eval_search.py**（检索质量：用已入库商品图当查询集）
...
- [ ] **Step 4: Commit** `git add -A && git commit -m "feat: 验收Harness（全场景+检索质量测评）"`
...
- [ ] **Step 3: Commit** `git add -A && git commit -m "feat: 部署脚本（清场+systemd+engine插件）"`
